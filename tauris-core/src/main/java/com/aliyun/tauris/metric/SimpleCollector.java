 ...
 */
public abstract class SimpleCollector<Child extends CollectorChild> extends Collector {
    protected final String fullname;
    protected final String help;
    protected final List<String> labelNames;

    protected final ConcurrentMap<LabelsKey, Child> children = new ConcurrentHashMap<LabelsKey, Child>();
    ;
    ...
    public Child labels(String... labelValues) {
        ...
        LabelsKey key = keyFactory.makeKey(labelValues);
    ...
    /**
     * 删除在millis时间之后没有过更新的child
     * @param millis timestamp in millis
     */
    public void clearIfLastModifiedBefore(long millis) {
        List<LabelsKey> willBeRemoved = new ArrayList<>();
        for (Map.Entry<LabelsKey, Child> es : children.entrySet()) {
            if (es.getValue().getLastModified() < millis) {
                willBeRemoved.add(es.getKey());
            }
        }
        for (LabelsKey lbs : willBeRemoved) {
            children.remove(lbs);
        }
    }
    ...
    public <T extends Collector> T setChild(Child child, String... labelValues) {
        ...
        children.put(keyFactory.makeKey(labelValues), child);
        return (T) this;
    }
