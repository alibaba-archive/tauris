...
public class HttpInput extends BaseStreamInput {
    ...
    private static Gauge   QUEUE_SIZE            = Gauge.build().name("input_http_queue_size").labelNames("id").help("http request queue size").create().register();
    ...
    private BlockingArrayQueue<Runnable> requestQueue;
    ...
    public void doInit() throws TPluginInitException {
        ...
        requestQueue = new BlockingArrayQueue<>(queueCapacity, queueCapacity);
        pool = new QueuedThreadPool(maxThreads, minThreads, idleTimeout, requestQueue);
    ...
    private class HttpInputServlet extends HttpServlet {
        ...
        public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException {
            ...
            QUEUE_SIZE.labels(id()).set(requestQueue.size());
        }
        ...
        private void handleGetAndHead(HttpServletRequest request, HttpServletResponse response, Map<String, Object> headers) {
            ...
            event.addMeta("path", request.getRequestURI());
            event.addMeta("method", request.getMethod());
            ...
            if (queryString != null) {
                ...
                event.addMeta("querystring", request.getQueryString());
            }
            try {
                putEvent(event);
        ...
        private void handlePostAnPut(HttpServletRequest req, HttpServletResponse response, Map<String, Object> headers, TDecoder codec) throws IOException {
            ...
            TScanner            scanner     = HttpInput.this.scanner.wrap(in).withCodec(codec, getEventFactory());
