    ...
    public List<MetricFamilySamples> collect() {
        ...
        for (Map.Entry<LabelsKey, Child> c : children.entrySet()) {
            ...
            for (int i = 0; i < v.buckets.length; ++i) {
                List<String> labelValuesWithLe = new ArrayList<String>(c.getKey().getKeys());
            ...
            samples.add(new MetricFamilySamples.Sample(fullname + "_count", labelNames, c.getKey().getKeys(), v.buckets[buckets.length - 1]));
