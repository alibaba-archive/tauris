...
import com.aliyun.tauris.metrics.CKMSQuantiles.Quantile;
...
public class Summary extends SimpleCollector<Summary.Child> {
    ...
    public List<MetricFamilySamples> collect() {
        ...
        for (Map.Entry<LabelsKey, Child> c : children.entrySet()) {
            ...
            for (Map.Entry<Double, Double> q : v.quantiles.entrySet()) {
                List<String> labelValuesWithQuantile = new ArrayList<String>(c.getKey().getKeys());
            ...
            samples.add(new MetricFamilySamples.Sample(fullname + "_count", labelNames, c.getKey().getKeys(), v.count));
            samples.add(new MetricFamilySamples.Sample(fullname + "_sum", labelNames, c.getKey().getKeys(), v.sum));
        }
