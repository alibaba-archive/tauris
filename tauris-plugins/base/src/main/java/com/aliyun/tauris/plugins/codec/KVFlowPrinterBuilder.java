 ...
 */
@Name("kvflow")
public class KVFlowPrinterBuilder implements TPrinterBuilder {
    ...
    public KVFlowPrinterBuilder() {
    }

    public TPrinter create(OutputStream out) {
        return new KVFlowPrinter(out);
    }
    ...
    public class KVFlowPrinter extends AbstractKVFlow implements TPrinter {
        ...
        public KVFlowPrinter(OutputStream output) {
            this.output = output;
            this.init();
        }

        public void init() {
            headerBuffer = ByteBuffer.allocate(HEADER_LEN);
            headerBuffer.put(MAGIC_NUMBER);
            headerBuffer.put(VERSION);
            keyCount = 0;
            keyIndexMap = new HashMap<>();
            keysBuffer = ByteBuffer.allocate(MAX_KEY_COUNT * MAX_KEY_LEN);
            bodyBuffer = ByteBuffer.allocate(INITIAL_BODY_SIZE);
            elementBuffer = ByteBuffer.allocate(maxElementSize);
        }
